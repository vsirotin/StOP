...
export class FaRunner {
    ...
    private readonly sfsm: Sfsm;
    private readonly signals: string[];
    private signalIndex = 0;
    private interpreter: CommandInterpreter | null = null;
    ...
    private handleCommand(command: string, data?: unknown): void {
        ...
        if (this.signalIndex >= this.signals.length) {
            throw new Error(
                `FaRunner: command "${command}" arrived but no interpreter is ` +
 ...
 */
function formatTrace(entries: LogEntry[]): string {
    return entries
        .map((entry) => {
