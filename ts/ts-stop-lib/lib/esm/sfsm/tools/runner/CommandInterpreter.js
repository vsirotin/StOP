...
export class CommandInterpreter {
     ...
     */
    constructor(commandToSignal) {
        // Store a defensive copy so later mutations of the caller's object
        // cannot change this interpreter's behaviour.
        this.commandToSignal = Object.assign({}, commandToSignal);
    }
    /**
    ...
    interpretCommand(command) {
        if (!(command in this.commandToSignal)) {
            throw new Error(`CommandInterpreter: no signal mapping for command "${command}"`);
        }
        return this.commandToSignal[command];
    }
}
