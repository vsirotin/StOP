...
This skill produces a complete SFSM model — `<project>-structure.json` and `<project>-behavior.json` — derived from the business model artifacts produced by the `business-modeler` skill.
...
2. Create initial `<project>-structure.json` from `swimlanes.md`:
...
4. Create empty `<project>-behavior.json` (`{}`).
...
4. **Add transition** to `<project>-behavior.json` (B-Rule-04). Re-sort FA group by from-state.
...
2. For each fully runnable path:
   ...
   - If the final state matches the last state in the trace path → PASS.
   - If not → investigate and fix before proceeding.
